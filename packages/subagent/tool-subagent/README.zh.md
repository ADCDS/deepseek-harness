...
每个结果都以可选的 `route` 字段携带该生效路由——无论是被覆盖的还是纯继承的——并投影进该调用的展示元数据，从而让 UI 能够报告哪个平台与模型服务了此次委派。仅当没有任何层提供完整路由时它才缺席。没有任何已渲染的结果文本发生变化；在 PTC 模式下，该字段属于模型读取的已声明输出 schema。
...
每个父级请求支付固定的 schema 成本；模型选择会增加三个参数。每个提供方实例增加一个 schema，每个可继续实例还增加一个简短的系统提示词 section。在 PTC 模式下，结果 schema 的三个分支各携带一个可选 `route` 对象，由 `ToolOutputMap` 前缀按实例渲染一次。
